...
def iterator_format(iterator, config)
  ...
  "Series: #{config[:series]}NEWInvocation: #{$step} of #{config[:parameter_space_size]}NEWProgress: #{($step/config[:parameter_space_size]).round(2)}%NEWTime left: #{seconds_left}NEWParameters:NEW#{iter}"
end
