...
def setup
  self.func(:add, :command) { |v| @result[:command] ||= "ping -c #{v.count} -s #{v.size} #{v.dns}" } # construct PING command with current combination of parameters
  self.func(:add, :raw_ping, hide: true) { |v| @result[:raw_ping] ||= `#{v.command} 2>&1` } # run PING and save its raw result
