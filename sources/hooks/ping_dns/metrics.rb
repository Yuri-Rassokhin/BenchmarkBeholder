...
def setup
  self.func(:add, :command) { |v| @temp[:command] ||= "ping -c #{v.count} -s #{v.size} #{v.dns}" } # construct PING command with current combination of parameters
  self.func(:add, :raw_ping, hide: true) { |v| @temp[:raw_ping] ||= `#{v.command} 2>&1` } # run PING and save its raw result
